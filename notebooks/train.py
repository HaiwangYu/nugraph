...
def main(args):
    ...
    # W&B paths before WandbLogger is constructed. Keep all W&B writes off $HOME.
    ...
    if batch is not None:
        ...
        # NEW: hard check that transform feature dim == --in-features (no silent override)
        try:
            x = batch["hit"].x
            ...
            if feat_dim != int(args.in_features):
                raise RuntimeError(
                    f"[FATAL] Transform produced hit.x dim={feat_dim} but you requested --in-features={args.in_features}. "
                    f"Fix by launching with the correct --in-features (or change transform config)."
                )
        except Exception as e:
            if global_rank == 0:
    ...
    common_kwargs = dict(
        ...
        use_sp_features=args.use_sp_features,
        use_vtx_features=args.use_vtx_features,
    )
    ...
    if model_name == "nugraph4":
        callbacks.extend(
            [
                pl.callbacks.ModelCheckpoint(
                    monitor="instance/f1-val",
                    mode="max",
                    save_top_k=1,
                    filename="best-instance-f1",
                    dirpath=ckpt_dir,
                ),
                pl.callbacks.ModelCheckpoint(
                    monitor="combined/semantic_instance_f1-val",
                    mode="max",
                    save_top_k=1,
                    filename="best-joint-f1",
                    dirpath=ckpt_dir,
                ),
            ]
        )
    ...
    if world_size > 1:
        ...
        # CRITICAL: external MPI already created processes (1 proc per GPU)
        devices = ranks_per_node if use_gpu else 0
